...
export default function Prognozi() {
  const [isMobile, setIsMobile] = useState(false);
  const { language } = useGlobalContext();
  const path = usePathname();
  const pathCity = path.split("/")[2];
  const {
  ...
  const { activeCity, setGlobalTwentyFourHours } = useGlobalContext();
  const [activeData, setActiveData] = useState<string>(
    TEXTS[language].twentyfourHours
  );
  useEffect(() => {
    if (pathCity) {
      window.history.pushState({}, "", `/prognozi/${pathCity}`);
      setActiveCity(pathCity);
    } else {
      window.history.pushState({}, "", "/prognozi/tbilisi");
    }
  }, []);
  const getTodayWeather = async () => {
    ...
    weatherServices.getHourlyWeather(setGlobalTwentyFourHours, activeCity);
  };
